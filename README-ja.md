...
## Model Weave とは何か
Model Weave は、構造化されたソフトウェアモデリングの機能を Obsidian に提供します。設計資産を人間が読みやすく、Git などのバージョン管理と相性が良く、さらに AI による実装支援の構造化コンテキストとして利用可能な状態に保つことができます。
...
### 必要なところから使える
すべてのモデル形式を最初に覚える必要はありません。単純なクラス定義でも、複雑なデータフローでも、現在のタスクに必要なモデルタイプから使い始めることができます。

### 手書きのラフから AI による緻密な詳細化まで
手書きによるラフなデザインから、AI を活用した緻密なモデリングまで幅広くサポートします。最初は単純な文章から始め、設計が成熟するにつれて構造化されたテーブルへと洗練させていくことができます。

### 設計を確認するための図を自動生成
テキストを書くことに集中してください。Model Weave がオーバービューの確認や関係性のチェックのための図（Custom または Mermaid レンダラーを使用）を自動的に描画します。

### 設計書を地図に、ソースを探すことができる
「Source Links」機能を使えば、設計ドキュメントをナビゲーションマップに変えることができます。ドキュメントから実際の実装ファイル（Vault 外のファイルも含む）へ直接ジャンプできます。
...
- `Model Weave: Open modeling preview for active file`: 表示中のモデルを図としてプレビューします。
...
## 次に読むもの
- はじめに - 最初の 5 分間のチュートリアル。
- コマンドガイド - テンプレートやツールの全リスト。
- フォーマットガイド - 各モデルタイプの書き方。
- サンプル - 設計の具体例。
- English README - English version of this document.
...
- Mermaid、SVG、PNG は生成された「表示」に過ぎません。
...
- 詳細は docs/V0.7-rendering-policy.md を参照してください。
...
### Source Links
外部の実装ファイルへのリンクを定義できます。相対パスは設定の `localSourceRoot` を基準に解決されます。
詳細は docs/formats/FORMAT-common-sections.md を参照。
