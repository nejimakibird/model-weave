...
1. `class` または `er_entity` の単体ファイルを 1 つ作る  
   クラスなら責務・属性・メソッド・関係を、ER ならテーブル・カラム・関係を記述します。
2. `class_diagram` または `er_diagram` を作り、`## Objects` に対象を並べる  
   図は個別ファイルを束ねて可視化するビューです。
...
- `class`, `class_diagram`, `er_entity`, `er_diagram` の最小例を見れば基本記法が分かります
