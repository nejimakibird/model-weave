...
全セルが空白のテーブルデータ行は無視されます。各セルを trim した結果すべて空の場合、その行は Obsidian のテーブル編集などで生じる入力ノイズとして扱われ、モデル行として生成されず、必須項目の診断対象にもなりません。

一方、一部のセルに値がある行は従来どおり検証対象です。たとえば `id` / `ref` が空でも `notes` など他のセルに値がある場合、その行は空行とはみなされず、該当項目が必須であれば診断対象になります。ヘッダー行と区切り行には影響しません。
