...
The following classes are currently intended to remain stable where possible. The complete internal DOM structure is not guaranteed, so snippets should prefer these classes over deep DOM selectors.
...
- Prefer `.model-weave-screen-details .model-weave-data-table` for lower Screen preview tables.
